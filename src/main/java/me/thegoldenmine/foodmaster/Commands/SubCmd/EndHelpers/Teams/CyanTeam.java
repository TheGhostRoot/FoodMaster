...
public class CyanTeam {
    private final FoodMaster plugin;

    public CyanTeam(FoodMaster main) {
        plugin = main;
    }
    ...
    public void Players(Player player) {
        ...
        if (plugin.deathmatch.getGroupPlayersInCyanTeam(player) != null && !plugin.deathmatch.getGroupPlayersInCyanTeam(player).isEmpty()) {
            for (UUID uuid1 : plugin.deathmatch.getGroupPlayersInCyanTeam(player)) {
                if (uuid1 != null) {
        ...
        if (groupPlayersInCyanTeam != null && !groupPlayersInCyanTeam.isEmpty()) {
            ...
            for (UUID uuid : plugin.playerGroup.getPlayersInGroupOfPlayer(player)) {
                if (uuid != null) {
                    ...
                    if (player1 != null && !groupPlayersInCyanTeam.contains(uuid)) {
