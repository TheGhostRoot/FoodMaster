...
public class MainTabCompletion implements TabCompleter {
    ...
    public MainTabCompletion(FoodMaster plugin) {
        this.plugin = plugin;
    }
            ...
            if (args[0].equalsIgnoreCase("set")) {
                ...
                if (sender instanceof Player) {
                    ...
                    if (player1.hasPermission("foodm.commands") && player1.hasPermission("foodm.kick")) {
                        ...
                        for (UUID uuid : plugin.getAllPlayersInGame()) {
                            Player player = Bukkit.getPlayer(uuid);
                            if (player != null) {
                                Player playerSender = (Player) sender;
                                if (!playerSender.getUniqueId().equals(player.getUniqueId())) {
                ...
                if (sender instanceof Player) {
                    ...
                    if (player.hasPermission("foodm.commands") && player.hasPermission("foodm.group")) {
                        if (plugin.isPlayerInGroup(player)) {
                            if (plugin.getPlayersInGroupOfPlayer(player).isEmpty()) {
                    ...
                    if (player.hasPermission("foodm.commands") && player.hasPermission("foodm.group")) {
                        for (Player player1 : Bukkit.getOnlinePlayers()) {
                            if (player1.hasPermission("foodm.commands") && !player.getUniqueId().equals(player1.getUniqueId())) {
                                boolean b = plugin.isPlayerInGroup(player1) && !plugin.getPlayersInGroupOfPlayer(player1).contains(player.getUniqueId());
                                if (b || !plugin.isPlayerInGroup(player1)) {
