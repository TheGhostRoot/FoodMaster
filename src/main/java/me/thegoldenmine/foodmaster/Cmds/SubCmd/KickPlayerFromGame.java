            ...
            if (playerKick != null) {
                ...
                if (plugin.kickedPlayers.contains(playerKick.getUniqueId())) {
                    ...
                    if (plugin.isPlayerInGroup(playerKick)) {
                        Set<UUID> groupClone = new HashSet<>(plugin.getPlayersInGroupOfPlayer(playerKick));
                        boolean end = false;
                        for (UUID uuid : groupClone) {
                            ...
                            if (playerInGroup != null) {
                                if (groupClone.size() == 2) {
                                    ...
                                    if (plugin.isPlayerInGame(playerKick)) {
                                        plugin.removePlayerFromGame(playerKick);
                                    }
                                    if (plugin.isPlayerInWaitingLobby(playerKick)) {
